...
核心要求：
1. text 为该分镜对应的原文片段，保留原文文字，不要做任何修改、润色、删减或添加。一个分镜中不超出3个句号 。 
...
3. **气泡**（面向读者）：承载原文中所有**对话**。格式：角色名-版本的气泡："对话内容"（使用资产列表中完整的角色名-版本格式）。
...
整体排版：
用一段话概述本页的排版/分格方案。**阅读动线固定为从左到右、从上到下**（即每行从左往右读，换行后继续从左往右读），排版布局必须严格符合此阅读方向。

**可灵活运用蒙太奇手法打破单调排版**，例如：
- 平行蒙太奇：同一时间不同人物的反应交替切格。
- 对比蒙太奇：将反差强烈的两个格相邻排列（如远景与特写、静与动）。
- 节奏蒙太奇：用短格快切营造紧张感，长格慢叙营造舒缓感。
- 交叉蒙太奇：不同空间/时间线交替推进叙事。
排版不求对称工整，**画面感优先**，只要阅读动线不乱即可。
...
- 气泡：角色名-版本的气泡："文字内容"。每个气泡文字不超过 2 行（约25个汉字），长文本拆成多个气泡依次排列
