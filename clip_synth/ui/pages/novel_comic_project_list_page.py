...
logger = logging.getLogger("clip_synth.novel_comic")
...
class NovelComicProjectCard(QFrame):
    ...
    def _setup_ui(self) -> None:
        ...
        icon_label = QLabel("\U0001f5bc")
        icon_label.setObjectName("novelComicCardIcon")
...
class NovelComicProjectListPage(QFrame):
    open_project = Signal(str)
    start_chapter_page = Signal(str)

    def __init__(
        self,
        novel_comic_state_service: NovelComicStateService,
        parent: QWidget | None = None,
    ):
        super().__init__(parent)
        self._novel_comic_state_service = novel_comic_state_service
        self._cards: list[NovelComicProjectCard] = []
        self.setObjectName("novelComicProjectListPage")
        self._setup_ui()
    ...
    def _setup_ui(self) -> None:
        ...
        new_project_btn = QPushButton("创建项目")
    ...
    def _load_projects(self) -> None:
        ...
        projects = self._novel_comic_state_service.list_projects()
        for project in projects:
            card = NovelComicProjectCard(
        ...
        if idx == 0:
            empty_label = QLabel("暂无项目，点击「创建项目」开始创建")
            empty_label.setObjectName("mixEmptyLabel")
            empty_label.setAlignment(Qt.AlignCenter)
            self._grid_layout.addWidget(empty_label, 0, 0, 1, 4)

    def _on_new_project(self) -> None:
        dialog = NovelComicNewProjectDialog(self)
        if dialog.exec() != QDialog.Accepted:
            return

        project = self._novel_comic_state_service.create_project(dialog.project_name)
        self.start_chapter_page.emit(project.id)
    ...
    def _on_delete_selected(self) -> None:
        ...
        for sid in selected_ids:
            if sid:
                self._novel_comic_state_service.delete_project(sid)
