...
class VideoCutWorker(QThread):
    ...
    def _process_single_video(self, video_path: str) -> None:
        from clip_synth.utils.gpu_accel import get_video_encoder_args, is_gpu_accel_enabled

        gpu_enabled = is_gpu_accel_enabled()

        encoder_args = get_video_encoder_args()
        logger.info("视频处理 GPU: enabled=%s, encoder_args=%s", gpu_enabled, encoder_args)
